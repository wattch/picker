...
function InnerRangePicker(props) {
  ...
  var _useState = (0, React.useState)(isNowValue || [false, false]),
  ...
  function triggerChange(newValue, sourceIndex, nowValueEnabled) {
    ...
    if (startValue && endValue && generateConfig.isAfter(startValue, endValue)) {
      ...
      picker !== 'week' && picker !== 'quarter' && picker !== 'time' && !(0, _dateUtil.isSameDate)(generateConfig, startValue, endValue)) {
        ...
        openRecordsRef.current = (0, _defineProperty2.default)({}, srcIndex, true);
    ...
    if (nextOpenIndex !== null && nextOpenIndex !== mergedActivePickerIndex && (!openRecordsRef.current[nextOpenIndex] || !(0, _miscUtil.getValue)(values, nextOpenIndex)) && (0, _miscUtil.getValue)(values, srcIndex)) {
  ...
  var _onTextChange = function onTextChange(newText, index) {
    ...
    if (inputDate && !disabledFunc(inputDate)) {
      ...
      if (index === 0 && generateConfig.isAfter(now, selectedValue[1])) {
        ...
        setSelectedValue((0, _miscUtil.updateValues)(selectedValue, now, 1));
        setViewDate(now, index);
